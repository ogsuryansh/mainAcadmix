    ...
    <section className="hero" ref={ref}>
      <div className={`hero-left reveal${visible ? ' visible' : ''}`}>
        ...
        <h1 className="hero-title">
          Everything You Need to<br />Crack <span>NEET</span>, <span>JEE</span> &amp; <span>Boards</span>
        </h1>
        <p className="hero-sub">
          Premium notes, digital books and test series — organized in one place. Purchase once, access forever.
        </p>
        <div className="hero-actions">
          <a href="#featured" className="btn-primary">Explore Resources</a>
          <a href="#tests" className="btn-outline">Browse Test Series</a>
        </div>
        <div className="hero-stats">
          ...
          <div className="hero-stat">
            ...
            <span className="hero-stat-lbl">Resources</span>
          </div>
          <div className="hero-stat-div" />
          <div className="hero-stat">
            <span className="hero-stat-num">{tests.toLocaleString()}+</span>
            <span className="hero-stat-lbl">Mock Tests</span>
          </div>
        </div>
      </div>
...
export default function Home() {
  ...
  const notes = products.filter(p => p.type === 'notes').slice(0, 4)
  const tests = products.filter(p => p.type === 'test_series').slice(0, 3)
  const books = products.filter(p => p.type === 'book').slice(0, 3)
    ...
    <>
      ...
      <section className="section" id="featured">
        <div className="section-head reveal">
          ...
          <p className="section-sub">Chapter-wise, exam-focused notes by subject experts</p>
        </div>
        <div className="cards-grid">
          {notes.length > 0 ? notes.map((n, i) => (
            <ProductCard
              key={n._id} product={n} delay={`${i * .07}s`}
              purchased={purchasedIds.has(n._id)}
              onAlreadyPurchased={showAlreadyPurchasedToast}
            />
          )) : <p style={{color:'#6B7280'}}>No notes available yet.</p>}
        </div>
      </section>

      {/* Countdowns */}
      <section className="section-sm" id="countdown">
        <div className="section-head reveal">
      ...
      <section className="section" id="tests">
        <div className="section-head reveal">
          <div className="section-tag">Test Series</div>
          <h2 className="section-title">Popular Test Series</h2>
          <p className="section-sub">NTA-pattern tests with detailed analysis and rank prediction</p>
        </div>
        <div className="cards-grid-3">
          {tests.length > 0 ? tests.map((t, i) => (
            <ProductCard
              key={t._id} product={t} delay={`${i * .07}s`}
              purchased={purchasedIds.has(t._id)}
              onAlreadyPurchased={showAlreadyPurchasedToast}
            />
          )) : <p style={{color:'#6B7280'}}>No test series available yet.</p>}
        </div>
      </section>

      {/* Books */}
      <section className="section-sm" id="books">
        <div className="section-head reveal">
          <div className="section-tag">Books & PDFs</div>
          <h2 className="section-title">Top Books &amp; eBooks</h2>
          <p className="section-sub">PYQ books, reference books and eBooks — instant digital access</p>
        </div>
        <div className="cards-grid-3">
          {books.length > 0 ? books.map((b, i) => (
            <ProductCard
              key={b._id} product={b} delay={`${i * .07}s`}
              purchased={purchasedIds.has(b._id)}
              onAlreadyPurchased={showAlreadyPurchasedToast}
            />
          )) : <p style={{color:'#6B7280'}}>No books available yet.</p>}
        </div>
      </section>
      ...
      <section className="section-sm">
        <div className="cta-wrap reveal">
          ...
          <div className="cta-btns">
            <a href="#featured" className="btn-white">Browse Resources</a>
            <a href="#tests" className="btn-outline-white">View Test Series</a>
          </div>
        </div>
      </section>
        ...
        <div className="footer-inner">
          <div className="footer-brand">
            <div className="footer-brand-logo">
              <div className="nav-logo-icon" style={{width:24,height:24,borderRadius:6,background:'#6D28D9',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:'.8rem',fontWeight:800}}>A</div>
              <span>Acadmix</span>
            </div>
          ...
          <div className="footer-col">
            <h4>Resources</h4>
            <a href="#">Notes</a><a href="#">Test Series</a><a href="#">Books</a><a href="#">PYQs</a>
          </div>
          <div className="footer-col">
            <h4>Exams</h4>
            <a href="#">NEET</a><a href="#">JEE Main</a><a href="#">JEE Advanced</a><a href="#">Boards</a>
          </div>
          <div className="footer-col">
            <h4>Company</h4>
            <a href="#">About</a><a href="#">Blog</a><a href="#">Contact</a><a href="#">Refund Policy</a>
          </div>
        </div>
